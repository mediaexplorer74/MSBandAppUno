...
using SLImage = Windows.UI.Xaml.Controls.Image; //using SLImage = SixLabors.ImageSharp.Image;
...
namespace Microsoft.Band
{
    public static class BandIconExtensions
    {
        public static BandIcon ToBandIcon(this ImageSource source)
        {
            ...
            {
                agnosticImage = default;//SLImage.Load(bitmap.UriSource.AbsolutePath);
            }
            else if (source is RenderTargetBitmap softwareBitmap)
            {
                IBuffer buffer = softwareBitmap.GetPixelsAsync().AsTask().Result;
                agnosticImage = default;//SLImage.LoadPixelData<Bgra32>(buffer.ToArray(), softwareBitmap.PixelWidth, softwareBitmap.PixelHeight);
            }
            ...
            return default;//new(convertedImage.Width * bytesPerPixel, convertedImage.Height * bytesPerPixel, bgr565Bytes);
        }
    }
}
