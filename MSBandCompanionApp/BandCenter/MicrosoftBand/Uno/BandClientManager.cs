...
namespace Microsoft.Band
{
    public class BandClientManager : IBandClientManager
    {
        ...
        {
            Guid serviceGuid = new(isBackground ? "{A502CA9B-2BA5-413C-A4E0-13804E47B38F}" 
                : "{A502CA9A-2BA5-413C-A4E0-13804E47B38F}");
            return Task.Run(() =>
            {
                return (IBandInfo[])BluetoothTransport.GetConnectedDevices(
                    serviceGuid, 
                    new LoggerProviderStub());
            });
        }
        ...
        public async Task<IBandClient> ConnectAsync(IBandInfo bandInfo)
        {
            ...
            try
            {
                ...
                deviceTransport = await BluetoothTransport.CreateAsync(bandInfo, 
                    loggerProvider, MaximumBluetoothConnectAttempts).ConfigureAwait(false);
                pushServiceTransport = new PushServiceTransport(bluetoothDeviceInfo, 
                    loggerProvider);
                client = new BandStoreClient(bluetoothDeviceInfo, deviceTransport, 
                    pushServiceTransport, loggerProvider, StoreApplicationPlatformProvider.Current);
