...
namespace Microsoft.Band.Admin
{
    ...
    {
        private static readonly MediaTypeHeaderValue OctetStreamContentTypeHeaderValue 
            = MediaTypeHeaderValue.Parse("application/octet-stream");

        private static readonly HttpHeader MSBlockBlobHeader = new HttpHeader("x-ms-blob-type", 
            "BlockBlob");
        ...
        internal CloudProvider(ServiceInfo serviceInfo)
        {
            ...
            {
                ArgumentException ex2 = new ArgumentException(SR.ServiceAddressIsMissing,
                    "serviceInfo");
            ...
            {
                ArgumentException ex3 = new ArgumentException(SR.AccessTokenIsMissing,
                    "serviceInfo");
            ...
            {
                ArgumentException ex4 = new ArgumentException(SR.DiscoveryServiceAddressIsMissing,
                    "serviceInfo");
            ...
            {
                ArgumentException ex5 = new ArgumentException(SR.DiscoveryServiceTokenIsMissing,
                    "serviceInfo");
            ...
            authorizationHeader = new HttpHeader(
                "Authorization", $"WRAP access_token=\"{serviceInfo.AccessToken}\"");
            authorizationHeaderForDiscoveryService = 
                $"WRAP access_token=\"{serviceInfo.DiscoveryServiceAccessToken}\"";
            profileUri = new Uri($"{serviceInfo.PodAddress}/v1/userprofiles");
            firmwareUpdateInfoUrlFormat =
                $"{serviceInfo.FileUpdateServiceAddress}/api/FirmwareQuery?deviceFamily={{0}}&publishType=Latest&OneBL={{1}}&TwoUp={{2}}&currentFirmwareVersion={{3}}&IsForcedUpdate={{4}}";
        ...
        internal FileUploadStatus UploadFileToCloud(Stream stream, LogFileTypes logType, 
            string uploadId, UploadMetaData metadata, CancellationToken cancellationToken)
        {
            if (stream == null)
            {
            ...
            if (logType != LogFileTypes.Telemetry && logType != LogFileTypes.CrashDump
                && logType != LogFileTypes.Sensor && logType != LogFileTypes.KAppLogs)
            {
                Exception ex5 = new ArgumentOutOfRangeException("logType", 
                    CommonSR.UnsupportedFileTypeForCloudUpload);
                Logger.LogException(LogLevel.Error, ex5);
                throw ex5;
            }
            ...
            Logger.Log(LogLevel.Info, "Uploading file to the cloud; Type: {0}, Upload Id: {1}, Size: {2}", 
                logType, uploadId, stream.Length);
            string requestUri = string.Format("{0}/v2/MultiDevice/UploadSensorPayload?logType={1}", 
                new object[2]
            {
            podAddress,
            ...
            using HttpResponseMessage httpResponseMessage = Send(httpClient, 
            ...
            switch (httpResponseMessage.StatusCode)
            {
                case HttpStatusCode.OK:
                    LogRequestAndResponseMessages(LogLevel.Verbose, httpRequestMessage, 
                ...
                case HttpStatusCode.Conflict:
                    LogRequestAndResponseMessages(LogLevel.Info, httpRequestMessage, 
                        httpResponseMessage, responseContent);
                    return FileUploadStatus.Conflict;
                case HttpStatusCode.ExpectationFailed:
                    LogRequestAndResponseMessages(LogLevel.Warning, httpRequestMessage, 
                        httpResponseMessage, responseContent);
                    return FileUploadStatus.UploadDone;
                default:
                    LogRequestAndResponseMessages(LogLevel.Error, httpRequestMessage, 
                        httpResponseMessage, responseContent);
                    throw CreateAppropriateException(httpResponseMessage, 
                        responseContent, string.Format(CommonSR.FileUploadToCloudFailed, new object[1] { logType }));
            }
        }

        internal Dictionary<string, LogUploadStatusInfo> GetLogProcessingUpdate(
            IEnumerable<string> uploadIDs, CancellationToken cancellationToken)
        {
            TimeSpan timeout = TimeSpan.FromSeconds(20.0);
            StringBuilder stringBuilder = new StringBuilder();
            bool flag = true;
            stringBuilder.AppendFormat("{0}/v2/MultiDevice/GetUploadStatus?uploadIds=", 
                new object[1] { podAddress });
            ...
            {
                LogRequestAndResponseMessages(LogLevel.Warning, requestMessage, 
                    httpResponseMessage, text);
                throw CreateAppropriateException(httpResponseMessage, text, 
                    CommonSR.LogProcessingStatusDownloadError);
            }
            LogRequestAndResponseMessages(LogLevel.Verbose, requestMessage, 
                httpResponseMessage, text);
            return CargoClient.DeserializeJson<Dictionary<string, 
                LogUploadStatusInfo>>(text);
        }

        internal bool DownloadFile(string downloadFileUrl, Stream updateStream, 
            TimeSpan timeout, params HttpHeader[] additionalHeaders)
        {
            return DownloadFile(downloadFileUrl, updateStream, timeout, 
                CancellationToken.None, (IEnumerable<HttpHeader>)additionalHeaders);
        }

        internal bool DownloadFile(string downloadFileUrl, Stream updateStream, 
            TimeSpan timeout, IEnumerable<HttpHeader> additionalHeaders)
        {
            return DownloadFile(downloadFileUrl, updateStream, timeout,
                CancellationToken.None, additionalHeaders);
        }

        internal bool DownloadFile(string downloadFileUrl, Stream updateStream, 
            TimeSpan timeout, CancellationToken cancel, params HttpHeader[] additionalHeaders)
        {
            return DownloadFile(downloadFileUrl, updateStream, timeout, cancel, 
                (IEnumerable<HttpHeader>)additionalHeaders);
        }

        internal bool DownloadFile(string downloadFileUrl, Stream updateStream, TimeSpan timeout, 
            CancellationToken cancel, IEnumerable<HttpHeader> additionalHeaders)
        {
            string responseContent = string.Empty;
        ...
        internal EphemerisCloudVersion GetEphemerisVersion(CancellationToken cancellationToken)
        {
            ...
            using HttpRequestMessage requestMessage = new HttpRequestMessage(
                HttpMethod.Get, ephemerisProfileUrl);
            using HttpResponseMessage httpResponseMessage = Send(httpClient, requestMessage, 
                cancellationToken);
            if (httpResponseMessage.Content != null)
            {
                responseContent = httpResponseMessage.Content.ReadAsStringAsync().Result;
            ...
            if (httpResponseMessage.StatusCode != HttpStatusCode.OK)
            {
                LogRequestAndResponseMessages(LogLevel.Warning, requestMessage, 
                    httpResponseMessage, responseContent);
                throw CreateAppropriateException(httpResponseMessage, responseContent, 
                    CommonSR.EphemerisVersionDownloadError);
            }
            LogRequestAndResponseMessages(LogLevel.Verbose, requestMessage,
                httpResponseMessage, responseContent);
        ...
        internal bool GetEphemeris(EphemerisCloudVersion ephemerisVersion, 
            Stream updateStream, CancellationToken cancel)
        {
            Logger.Log(LogLevel.Info, "Downloading ephemeris data from the cloud");
            TimeSpan timeout = TimeSpan.FromMinutes(2.0);
            return DownloadFile(ephemerisVersion.EphemerisProcessedFileDataUrl, 
                updateStream, timeout, cancel);
        }

        internal TimeZoneDataCloudVersion GetTimeZoneDataVersion(IUserProfile profile,
            CancellationToken cancellationToken)
        {
            string responseContent = string.Empty;
            TimeSpan timeout = TimeSpan.FromSeconds(20.0);
            using HttpClient httpClient = CreateHttpClient(
                EnumerableExtensions.Concat(authorizationHeader, 
                GetLocalizationHeadersFromProfile(profile)));
            httpClient.Timeout = timeout;
            using HttpRequestMessage requestMessage = new HttpRequestMessage(
                HttpMethod.Get, timezoneUpdateInfoUrl);
            using HttpResponseMessage httpResponseMessage = Send(httpClient, 
                requestMessage, cancellationToken);
            if (httpResponseMessage.StatusCode != HttpStatusCode.OK)
            {
                ...
                LogRequestAndResponseMessages(LogLevel.Warning, requestMessage, 
                    httpResponseMessage, responseContent);
                throw CreateAppropriateException(httpResponseMessage, responseContent, 
                    CommonSR.TimeZoneDataVersionDownloadError);
            }
        ...
        {
            TimeSpan timeout = TimeSpan.FromMinutes(2.0);
            return DownloadFile(timeZoneDataVersion.Url, updateStream, timeout, 
                GetLocalizationHeadersFromProfile(profile));
        }

        internal FirmwareUpdateInfo GetLatestAvailableFirmwareVersion(
            FirmwareVersions deviceVersions, bool firmwareOnDeviceValid, List<KeyValuePair<string, string>> queryParams, CancellationToken cancellationToken)
        {
            string responseContent = string.Empty;
            TimeSpan timeout = TimeSpan.FromSeconds(20.0);
            ...
            if (queryParams != null)
            {
                ...
                if (queryParams != null)
                {
                    ...
                    {
                        if (string.Compare(queryParam.Key, 
                            "debug.force", StringComparison.OrdinalIgnoreCase) == 0)
                        {
                            if (string.Compare(queryParam.Value, 
                                "true", StringComparison.OrdinalIgnoreCase) == 0)
                            {
                                Logger.Log(LogLevel.Warning, 
                                    "Firmware check parameter Debug.Force set");
                        ...
                        else
                        {
                            stringBuilder.AppendFormat(
                                "&{0}={1}", new object[2] { queryParam.Key, queryParam.Value });
                        }
                    }
                }
            }
            stringBuilder2.AppendFormat(firmwareUpdateInfoUrlFormat, 
                deviceVersions.PcbId, deviceVersions.BootloaderVersion, 
                deviceVersions.UpdaterVersion, deviceVersions.ApplicationVersion, 
            ...
            Logger.Log(LogLevel.Info, 
                "Getting latest available firmware version from cloud: deviceFamily: {0}, OneBLVersion: {1}, TwoUpVersion: {2}, currentFirmwareVersion: {3}, IsForcedUpdate: {4}", deviceVersions.PcbId, deviceVersions.BootloaderVersion, deviceVersions.UpdaterVersion, deviceVersions.ApplicationVersion, firmwareOnDeviceValid ? "false" : "true");
            ...
            using (HttpClient httpClient = CreateHttpClient(authorizationHeader))
            {
                ...
                if (httpResponseMessage.StatusCode != HttpStatusCode.OK)
                {
                    LogRequestAndResponseMessages(LogLevel.Warning, requestMessage,
                        httpResponseMessage, responseContent);
                    throw CreateAppropriateException(httpResponseMessage, responseContent, 
                        CommonSR.FirmwareUpdateInfoError);
                }
            ...
            {
                Logger.Log(LogLevel.Info, 
                    "Firmware availability: {0} version from cloud: deviceFamily: {1}, Version: {2}", 
                    firmwareUpdateInfo.IsFirmwareUpdateAvailable, firmwareUpdateInfo.DeviceFamily, 
                    firmwareUpdateInfo.FirmwareVersion);
            }
            else
            {
                Logger.Log(LogLevel.Info, 
                    "Firmware availability: {0}", firmwareUpdateInfo.IsFirmwareUpdateAvailable);
            }
            if (!firmwareOnDeviceValid && !firmwareUpdateInfo.IsFirmwareUpdateAvailable)
            {
                Logger.Log(LogLevel.Warning, 
                    "Reported device firmware invalid, but cloud did not honor it");
            }
        ...
        internal void GetFirmwareUpdate(FirmwareUpdateInfo updateInfo, Stream updateStream, CancellationToken cancellationToken)
        {
            ...
            Logger.Log(LogLevel.Info, 
                "Attempting to download firmware update into a local file from the cloud");
            string[] array = new string[3] { updateInfo.PrimaryUrl, 
                updateInfo.MirrorUrl, updateInfo.FallbackUrl };
            bool flag = false;
            StringBuilder stringBuilder = new StringBuilder();
            ...
            if (!flag)
            {
                BandCloudException ex2 = new BandCloudException(string.Format(
                    CommonSR.FirmwareUpdateDownloadError, new object[1] { stringBuilder }));
                Logger.LogException(LogLevel.Error, ex2);
                throw ex2;
            }
        }
        ...
        internal void SaveUserProfile(CloudProfile profile, bool createNew, 
            CancellationToken cancellationToken)
        {
            string responseContent = string.Empty;
            ...
            using HttpRequestMessage httpRequestMessage = new HttpRequestMessage(
                createNew ? HttpMethod.Post : HttpMethod.Put, string.Format("{0}/{1}", new object[2]
            {
            profileUri,
            createNew ? "post" : "put"
            }));
            ...
            if (httpResponseMessage.StatusCode != HttpStatusCode.OK)
            {
                LogRequestAndResponseMessages(LogLevel.Warning, httpRequestMessage, 
                    httpResponseMessage, responseContent);
                throw CreateAppropriateException(httpResponseMessage, responseContent, 
                    CommonSR.WriteProfileFailed);
            }
            LogRequestAndResponseMessages(LogLevel.Verbose, httpRequestMessage, 
                httpResponseMessage, responseContent);
        }

        internal void SaveDeviceLinkToUserProfile(CloudProfileDeviceLink profile, 
            CancellationToken cancellationToken)
        {
            string responseContent = string.Empty;
            ...
            if (httpResponseMessage.StatusCode != HttpStatusCode.OK)
            {
                LogRequestAndResponseMessages(LogLevel.Warning, 
                    httpRequestMessage, httpResponseMessage, responseContent);
                throw CreateAppropriateException(httpResponseMessage, 
                    responseContent, CommonSR.WriteProfileFailed);
            }
            LogRequestAndResponseMessages(LogLevel.Verbose, 
                httpRequestMessage, httpResponseMessage, responseContent);
        }

        internal void SaveUserProfileFirmware(byte[] firmwareBytes, 
            CancellationToken cancellationToken)
        {
            string responseContent = string.Empty;
            ...
            using HttpRequestMessage httpRequestMessage = 
                new HttpRequestMessage(HttpMethod.Put, $"{profileUri}/put");
            httpRequestMessage.Content = CreateJsonContent(json);
            using HttpResponseMessage httpResponseMessage = Send(httpClient, 
                httpRequestMessage, cancellationToken);
            if (httpResponseMessage.Content != null)
        ...
        {
            yield return new HttpHeader("Accept-Language", 
                profile.DeviceSettings.LocaleSettings.Language.ToLanguageCultureName());
            yield return new HttpHeader("Region", 
                profile.DeviceSettings.LocaleSettings.LocaleId.ToRegionName());
        }
        ...
        private HttpClient CreateHttpClient(IEnumerable<HttpHeader> additionalHeaders)
        {
            ...
            if (additionalHeaders != null)
            {
                ...
                {
                    if (string.Compare(userAgentHeader.Name, additionalHeader.Name, 
                        StringComparison.OrdinalIgnoreCase) != 0)
                    {
                        httpClient.DefaultRequestHeaders.Add(additionalHeader.Name, 
        ...
        private HttpResponseMessage Send(HttpClient client, HttpRequestMessage requestMessage, 
            CancellationToken cancellationToken)
        {
            try
            {
        ...
        private static BandHttpException CreateAppropriateException(HttpResponseMessage responseMessage, 
            string responseContent, string message, Exception innerException = null)
        {
            HttpStatusCode statusCode = responseMessage.StatusCode;
            if (statusCode == HttpStatusCode.Unauthorized || statusCode == HttpStatusCode.Forbidden)
            {
                return new BandHttpSecurityException(responseContent, FormatMessage(responseMessage, 
                    responseContent, message), innerException);
            }
            return new BandHttpException(responseContent, FormatMessage(responseMessage, 
                responseContent, message), innerException);
        }

        private static string FormatMessage(HttpResponseMessage responseMessage, 
            string responseContent, string message)
        {
            ...
            {
                stringWriter.WriteLine(" {0}: {1} {2} {3}", CommonSR.HttpExceptionStatusLineLabel, 
        ...
        private void LogRequestAndResponseMessages(LogLevel level, HttpRequestMessage requestMessage, 
            HttpResponseMessage responseMessage)
        {
            lock (debugLock)
            {
                Logger.Log(level, "Request: {0} {1}", requestMessage.Method, requestMessage.RequestUri);
                Logger.Log(level, "Response StatusCode: {0} ({1})", responseMessage.StatusCode, 
                    (int)responseMessage.StatusCode);
            }
        }

        private void LogRequestAndResponseMessages(LogLevel level, HttpRequestMessage requestMessage, 
            HttpResponseMessage responseMessage, string responseContent)
        {
            lock (debugLock)
            {
        ...
        private void LogRequestAndResponseMessages(LogLevel level, HttpRequestMessage requestMessage, 
            HttpResponseMessage responseMessage, Stream responseStream)
        {
            lock (debugLock)
            {
                LogRequestAndResponseMessages(level, requestMessage, responseMessage);
