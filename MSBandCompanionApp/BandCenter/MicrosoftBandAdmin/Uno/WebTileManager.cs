...
namespace Microsoft.Band.Admin
{
    ...
    internal class WebTileManager : IWebTileManager
    {
        ...
        public async Task<IWebTile> GetWebTilePackageAsync(Uri uri)
        {
            ...
            try
            {
                using (Stream zipStream = await Task.Run(() => storageProvider.OpenFileForRead(
                    StorageProviderRoot.App, zipPath, -1)))
                {
                    webTile = await GetWebTilePackageAsync(zipStream, url);
                    webTile.RequestHeaders = WebTileAgentHelper.GetAgentHeadersForUrl(
                        url, webTile.Organization);
                }
                return webTile;
            }
            finally
        ...
        public async Task<IWebTile> GetWebTilePackageAsync(Stream source, string sourceFileName)
        {
            IWebTile webTile = await Task.Run(delegate
            {
                ...
                try
                {
                    IWebTile webTile2 = DeserializeWebTilePackageFromJson(Path.Combine(new string[2] {
                    "Temp_WebTile", "manifest.json" }));
                    webTile2.Validate();
                    return webTile2;
                }
                catch
                {
                    TryDeleteFolder(StorageProviderRoot.App, "Temp_WebTile",
                        "temp folder after failed JSON deserialize");
                    throw;
                }
            });
        ...
        private void InstallWebTile(IWebTile webTile)
        {
            ...
            storageProvider.MoveFolder(StorageProviderRoot.App, "Temp_WebTile", StorageProviderRoot.App,
                text2);
            ...
            if (webTile.RequestHeaders != null && webTile.RequestHeaders.Count() > 0)
            {
                ...
                using Stream outputStream = storageProvider.OpenFileForWrite(StorageProviderRoot.App,
                    relativePath, append: false);
        ...
        public IList<Guid> GetInstalledWebTileIds()
        {
            ...
            if (!storageProvider.DirectoryExists(StorageProviderRoot.App, "WebTiles")
                    || !storageProvider.DirectoryExists(StorageProviderRoot.App,
                    WebTileInstalledFolder))
            {
                return list;
            }
        ...
        public IWebTile GetWebTile(Guid tileId)
        {
            ...
            {
                TryDeleteFolder(StorageProviderRoot.App, text,
                    "folder for installed webtile with missing package path");
                return null;
            }
            try
            {
                IWebTile webTile = DeserializeWebTilePackageFromJson(Path.Combine(new string[2]
                { text2, "manifest.json" }));
                ...
                {
                    using Stream inputStream = storageProvider.OpenFileForRead(StorageProviderRoot.App,
                        relativePath);
            ...
            {
                TryDeleteFolder(StorageProviderRoot.App, text,
                    "folder for installed webtile after failed JSON deserialize");
                throw;
            }
        }
        ...
        private IWebTile DeserializeWebTilePackageFromJson(Stream stream, int bufferSize = 8192)
        {
            using StreamReader reader = new StreamReader(stream, Encoding.UTF8,
                detectEncodingFromByteOrderMarks: false, bufferSize, leaveOpen: true);
        ...
        public async Task<string> DownloadWebTileAsync(string url, CancellationToken cancellationToken)
        {
            ...
            try
            {
                updateStream = await Task.Run(() => storageProvider.OpenFileForWrite(StorageProviderRoot.App,
                    localWebTileTempFileRelativePath, append: false));
            }
            catch (Exception innerException)
            {
                BandException ex = new BandException(string.Format(CommonSR.WebTileDownloadTempFileOpenError,
                    new object[1] { localWebTileTempFileRelativePath }), innerException);
            ...
            {
                TryDeleteFolder(StorageProviderRoot.App, "Temp_Zip",
                    "the temporary webtile file, after an error downloading it");
                throw;
            }
            transferTimer.Stop();
        ...
        private async Task DownloadFileAsync(string downloadFileUrl, Stream updateStream,
            TimeSpan timeout, CancellationToken cancellationToken)
        {
            ...
            using HttpResponseMessage responseMessage = await SendAsync(client, requestMessage,
                cancellationToken);
        ...
        private Task<HttpResponseMessage> SendAsync(HttpClient client, HttpRequestMessage requestMessage,
        ...
        {
            return new BandHttpException(responseContent, FormatMessage(responseMessage, responseContent,
                message), innerException);
        }

        private static string FormatMessage(HttpResponseMessage responseMessage, string responseContent,
            string message)
        {
        ...
        private string GetWebTileUrlFromUri(Uri uri)
        {
            ...
            if (firstValueByName != null && firstValueByName.Equals("download-manifest",
