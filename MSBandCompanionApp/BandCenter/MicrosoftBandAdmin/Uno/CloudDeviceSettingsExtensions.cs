...
namespace Microsoft.Band.Admin
{
    ...
    internal static class CloudDeviceSettingsExtensions
    {
        internal static DeviceSettings ToDeviceSettings(this CloudDeviceSettings cloudDeviceSettings)
        {
            ...
            {
                cloudDeviceSettings.DeviceName = 
                    cloudDeviceSettings.DeviceName.TruncateTrimDanglingHighSurrogate(16);
            }
            ...
            deviceSettings.TelemetryEnabled = cloudDeviceSettings.TelemetryEnabled.HasValue 
                && cloudDeviceSettings.TelemetryEnabled.Value;
            ...
            {
                deviceSettings.FirmwareByteArray = Convert.FromBase64String(
                    cloudDeviceSettings.FirmwareByteArray);
            }
            return deviceSettings;
        }
        ...
        {
            if (allCloudDeviceSettings == null || allCloudDeviceSettings.Count == 0 
                || allCloudDeviceSettings.First().Value == null)
            {
                return new Dictionary<Guid, DeviceSettings>();
            }
            return allCloudDeviceSettings.ToDictionary((KeyValuePair<Guid, CloudDeviceSettings> p) 
                => p.Key, (KeyValuePair<Guid, CloudDeviceSettings> p) => p.Value.ToDeviceSettings());
        }
    }
}
