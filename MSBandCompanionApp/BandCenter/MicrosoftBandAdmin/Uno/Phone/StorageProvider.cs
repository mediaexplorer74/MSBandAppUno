...
namespace Microsoft.Band.Admin.Phone
{
    ...
    internal sealed class StorageProvider : IStorageProvider
    {
        ...
        internal static void CleanDeprecated()
        {
            ...
            // FIXME
            /*
            IReadOnlyList<StorageFolder> result = 
                WindowsRuntimeSystemExtensions.AsTask<IReadOnlyList<StorageFolder>>(
                    localFolder.GetFoldersAsync()).Result;
                ...
                if (result[i].Name.Equals(text, StringComparison.OrdinalIgnoreCase))
                {
                    WindowsRuntimeSystemExtensions.AsTask(
                        WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(
                            localFolder.CreateFolderAsync(text, (CreationCollisionOption)3))
                        .Result.DeleteAsync((StorageDeleteOption)1)).Wait();
                    break;
                }
            }
            */
        ...
        internal static StorageProvider Create(string userId, string deviceId)
        {
            ...
            if (text.IndexOfAny(invalidFileNameChars) != -1)
            {
                ArgumentException ex2 = new ArgumentException(string.Format(
                    CommonSR.InvalidCharactersInFolderName, new object[1] { text }));
                Logger.LogException(LogLevel.Error, ex2);
                throw ex2;
            }
            if (text2.IndexOfAny(invalidFileNameChars) != -1)
            {
                ArgumentException ex3 = new ArgumentException(string.Format(
                    CommonSR.InvalidCharactersInFolderName, new object[1] { text2 }));
            ...
            return default;//new StorageProvider(WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(result.CreateFolderAsync(text2, (CreationCollisionOption)3)).Result);
        }
        ...
        public Stream OpenFileForWrite(StorageProviderRoot root, string relativePath, bool append, int bufferSize = 0)
        {
            ...
            return default;//WindowsRuntimeStreamExtensions.AsStream((IRandomAccessStream)(FileRandomAccessStream)obj.GetResults(), bufferSize);
        }
        ...
        public IRandomAccessStream OpenFileRandomAccessStreamForRead(StorageProviderRoot root, string relativePath)
        {
            ...
            //WindowsRuntimeSystemExtensions.AsTask<IRandomAccessStream>(obj).Wait();
            return default;//(IRandomAccessStream)(FileRandomAccessStream)obj.GetResults();
        }
        ...
        public Stream OpenFileForRead(StorageProviderRoot root, string relativePath, int bufferSize = 0)
        {
            ...
            FileRandomAccessStream val = (FileRandomAccessStream)OpenFileRandomAccessStreamForRead(
                root, relativePath);
            if (bufferSize == -1)
            {
            ...
            //FIXME
            return default;//WindowsRuntimeStreamExtensions.AsStream((IRandomAccessStream)(object)val, bufferSize);
        }
        ...
        public void RenameFile(string relativeSourcePath, string relativeDestFolder, 
            string destFileName)
        {
            RenameFile(StorageProviderRoot.Device, relativeSourcePath, 
                StorageProviderRoot.Device, relativeDestFolder, destFileName);
        }

        public void RenameFile(StorageProviderRoot sourceRoot, string relativeSourcePath, 
            StorageProviderRoot destRoot, string relativeDestFolder, string destFileName)
        {
            Logger.Log(LogLevel.Verbose, "Renaming phone file:{0} to destinationFolder:{1}," +
                " destinationFileName:{2}", relativeSourcePath, relativeDestFolder, destFileName);
            //FIXME
         /*
            StorageFile result = WindowsRuntimeSystemExtensions.AsTask<StorageFile>(
                GetRootFolder(sourceRoot).GetFileAsync(relativeSourcePath)).Result;
            StorageFolder val = ResolveFolder(destRoot, relativeDestFolder);
            WindowsRuntimeSystemExtensions.AsTask<StorageFile>(result.CopyAsync
                ((IStorageFolder)(object)val, destFileName, (NameCollisionOption)1)).Wait();
            WindowsRuntimeSystemExtensions.AsTask(result.DeleteAsync()).Wait();
         */
        }
        ...
        public long GetFileSize(StorageProviderRoot root, string relativePath)
        {
            ...
            try
            {
                Logger.Log(LogLevel.Verbose, "Retrieved file size for phone file:{0} is {1}", 
                    relativePath, (long)((IRandomAccessStream)result).Size);
        ...
        public string[] GetFiles(StorageProviderRoot root, string folderRelativePath)
        {
            ...
            string[] array = default;//new string[result.Count];
        ...
        {
            Logger.Log(LogLevel.Verbose, "Checking if phone file exists with relative path: {0}", 
                relativePath);
            string text = Path.Combine(new string[2]
            {
            GetRootFolder(root).Path,
            ...
            {
                //FIXME
                return default;//WindowsRuntimeSystemExtensions.AsTask<StorageFile>(
                       //StorageFile.GetFileFromPathAsync(text)).Result != null;
            }
            catch (FileNotFoundException)
            {
                return false;
        ...
        public bool DirectoryExists(StorageProviderRoot root, string relativePath)
        {
            ...
            try
            {
                return default;//WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(
                    //StorageFolder.GetFolderFromPathAsync(text)).Result != null;
            }
            catch (FileNotFoundException)
            {
                return false;
        ...
        private StorageFolder ResolveFolder(StorageProviderRoot root, string folderRelativePath)
        {
            ...
            foreach (string text in array)
            {
                ...
                {
                    //FIXME
                    val = default;//WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(val.GetFolderAsync(text)).Result;
                }
            }
            return val;
        }

        public void MoveFolder(StorageProviderRoot sourceRoot, string relativeSourceFolder, 
            StorageProviderRoot destRoot, string relativeDestFolder, bool overwrite = false)
        {
            Logger.Log(LogLevel.Verbose, "Move folder:{0} to destinationFolder:{1}",
                relativeSourceFolder, relativeDestFolder);
            ...
            //FIXME
            StorageFolder result = default;//WindowsRuntimeSystemExtensions.AsTask<StorageFolder>(GetRootFolder(destRoot).CreateFolderAsync(relativeDestFolder, (CreationCollisionOption)2)).Result;
