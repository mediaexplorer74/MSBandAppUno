...
namespace Microsoft.Band.Admin.WebTiles
{
    ...
    internal class WebTileSyncInfo
    {
        ...
        private string LastSyncMappingsSerialized
        {
            get
            {
                return string.Join("__*$$/_MSeparatorS/_$$*__",
                    lastSyncMappings.Select((Dictionary<string, string> m) => string.Join("__*$$/_KSeparatorV/_$$*__", MappingAsStrings(m))));
            }
            set
            {
                ...
                string[] array = value.Split(new string[1] { "__*$$/_MSeparatorS/_$$*__" },
                    StringSplitOptions.None);
                for (int i = 0; i < array.Length; i++)
                {
                    string[] array2 = array[i].Split(new string[1] { "__*$$/_KSeparatorV/_$$*__" },
                        StringSplitOptions.None);
        ...
        {
            return lastSyncMappings.SelectMany((Dictionary<string, string> m)
                => MappingAsStrings(m)).SequenceEqual(other.SelectMany((Dictionary<string, string> m)
                => MappingAsStrings(m)));
        }
