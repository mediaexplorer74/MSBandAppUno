    ...
    public class WebTileResource : IWebTileResource
    {
        ...
        public Dictionary<string, string> Content
        {
            ...
            set
            {
                ...
                validator.CheckProperty("Content", value != null && value.Count > 0,
                    CommonSR.WTMissingVariableDefinitions);
                if (value != null)
                {
                    foreach (KeyValuePair<string, string> item in value)
                    {
                        ...
                        if (!flag)
                        {
                            validator.CheckProperty("Content", flag, string.Format(
                                CommonSR.WTInvalidVariableName, new object[1] { item.Key }));
                            break;
                        }
                        ...
                        {
                            validator.CheckProperty("Content", flag, string.Format(
                                CommonSR.WTInvalidVariableExpression, new object[1] { item.Key }));
                            break;
                        }
                    }
                }
            ...
            try
            {
                ...
                if (responseMessage.Content != null)
                {
                    if (responseMessage.Content.Headers != null
                            ...
                            && responseMessage.Content.Headers.ContentType.CharSet != null)
                    {
                        string charSet
                                = responseMessage.Content.Headers.ContentType.CharSet.Replace("\"", "");
                ...
                {
                    throw new BandHttpException(urlContent, CommonSR.WTFailedToFetchResourceData,
                        innerException);
                }
            }
            catch (Exception e)
        ...
        public async Task<bool> AuthenticateAsync()
        {
            using (HttpResponseMessage responseMessage = await GetResourceResponseAsync())
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new BandHttpException(await responseMessage.Content.ReadAsStringAsync(),
                        CommonSR.WTFailedToFetchResourceData, e);
                }
            }
            return true;
        }
        ...
        private async Task<HttpResponseMessage> GetResourceResponseAsync()
        {
            ...
            if (RequestHeaders != null)
            {
                ...
                for (int i = 0; i < requestHeaders.Length; i++)
                {
                    ...
                    {
                        throw new BandHttpException(Url, string.Format(CommonSR.WTBadHTTPRequestHeader,
                            new object[2] { headerNameValuePair.name, headerNameValuePair.value }),
                            innerException);
                    }
                }
            }
        ...
        internal string GetUniqueItemId(XmlNamespaceHelper xmlnshelper, bool isRSS, IXmlNode node)
        {
            ...
            if (text == null)
            {
                string text2 = xmlnshelper.ResolveNodeWithNamespace(node, isRSS
                    ? "description"
                    : "summary");
                if (text2 == null)
                {
        ...
        internal List<Dictionary<string, string>> GetFeedContentMappings(XmlDocument data)
        {
            ...
            foreach (KeyValuePair<string, string> item in Content)
            {
                ...
                {
                    string text =
                            xmlNamespaceHelper.ResolveNodeWithNamespace((IXmlNode)(object)data.DocumentElement,
                            item.Value);
                    if (text != null)
                    {
        ...
        internal void ResolveContentMappings(Dictionary<string, string> mappings, XmlDocument document)
        {
            ...
            {
                string text
                        = xmlNamespaceHelper.ResolveNodeWithNamespace((IXmlNode)(object)document.DocumentElement,
                        item.Value);
                if (text != null)
                {
